...
#if defined(NDEBUG)
# error "Dubeucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd9b4c5882350c4b4b9ffefe60376488082cea5717b3a3fca4a5656555295ad8f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dubeucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dubeucoin Signed Message:\n";
...
{
    // Dubeucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dubeucoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Dubeucoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Dubeucoin: 30 secounds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dubeucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dubeucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd9b4c5882350c4b4b9ffefe60376488082cea5717b3a3fca4a5656555295ad8f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/Nov/2014 Secretary of Defense Hagel announces his resignation";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x169ba15555b6d51d51ce65bbe14f08785285d75769f93ae9c2cc6ff3076a9198"));
...
unsigned char pchMessageStart[4] = { 0xf2, 0xe0, 0xc6, 0xdb }; // Dubeucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DubeucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DubeucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DubeucoinMiner(CWallet *pwallet)
{
    printf("DubeucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Dubeucoin-miner");
    ...
    try { loop {
        ...
        printf("Running DubeucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DubeucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DubeucoinMiner, pwallet));
}
